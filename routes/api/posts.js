...
router.get("/", (req, res) => {
  Post.find()
    ...
    .catch((err) => res.status(404).json({ error: "No posts was that Id" }));
});
...
router.get("/:id", (req, res) => {
  Post.findById(req.params.id)
    .then((posts) => res.json(posts))
    .catch((err) => res.status(404).json({ error: "No post was that Id" }));
});
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then((profile) => {
      Post.findById(req.params.id)
        ...
        .catch((err) => res.status(400).json({ error: "No Post found" }));
    });
  }
);
      ...
      Post.findById(req.params.id)
        .then((post) => {
          if (
            ...
            return res
              .status(400)
              .json({ error: "You have not yet linked this post" });
          }
